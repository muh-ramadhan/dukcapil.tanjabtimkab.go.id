...
function a_simpan()
    {
		...
		{	
			...
			$this->M_dataadmin->query_manual("INSERT INTO menu(id_parent,
									id_position,
                                    nama_menu,
									aktif,
									tgl_posting,
	...
	function a_edit()
    {
		...
		{  
			...
			$this->M_dataadmin->query_manual("UPDATE menu SET id_parent  = '".$this->input->post('id_parent')."',
									id_position = '".$this->input->post('posisi')."',
	...
	function urutall()
    {
		...
		{   
				...
				if ($leveluser=="admin") {
					for($i=0;$i<$jumlah;$i++){ 	
						$this->M_dataadmin->query_manual("UPDATE menu SET urutan = '".$urutan[$i]."' WHERE id_menu = '".$cek[$i]."'");   
	...
	function hapuslink()
    {
		...
		{  
			...
			$row = $query->row();
			if ($this->session->userdata('id_user') == $row->username or $leveluser=="admin") {  
				$this->M_dataadmin->query_manual("UPDATE menu SET link='' WHERE id_menu=".$kode."");  
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."menu/edit/".$kode."'>";
			}
		else {
			$this->denied();
		}  
