...
function index()
    {
		...
		else if ($leveluser=="user" || $leveluser=="view")
		{ 
...
function edit()
	...
	function deleteall()
    {
		...
		{   
				...
				if ($leveluser=="admin") {
					for($i=0;$i<$jumlah;$i++){ 
						$this->M_dataadmin->query_manual("DELETE FROM users WHERE id_users='".$cek[$i]."'");    
					}  
				}
				else {
					for($i=0;$i<$jumlah;$i++){
						$this->M_dataadmin->query_manual("DELETE FROM users WHERE username='".$this->session->userdata('id_user')."' and id_users='".$cek[$i]."'");  
					} 
				} 
				}
				//REDIRECT AWAL
