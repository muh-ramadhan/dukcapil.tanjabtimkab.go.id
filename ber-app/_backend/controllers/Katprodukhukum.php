...
function aktif()
	...
	function deleteall()
    {
		...
		{   
				...
				if ($leveluser=="admin") {
					for($i=0;$i<$jumlah;$i++){ 
						$this->M_dataadmin->query_manual("DELETE FROM katprodukhukum WHERE id_katprodukhukum='".$cek[$i]."'");    
					}  
				}
				else {
					for($i=0;$i<$jumlah;$i++){
						$this->M_dataadmin->query_manual("DELETE FROM katprodukhukum WHERE username='".$this->session->userdata('id_user')."' and id_katprodukhukum='".$cek[$i]."'");  
					} 
				} 
				}
				//REDIRECT AWAL
	...
	function urutall()
    {
		...
		{   
			...
			if ($leveluser=="admin") {
				for($i=0;$i<$jumlah;$i++){ 	
					$this->M_dataadmin->query_manual("UPDATE katprodukhukum SET urutan = '".$urutan[$i]."' WHERE id_katprodukhukum = '".$cek[$i]."'"); 				
				}  
			}
			else {
				$this->redirect();
