...
function index()
    {
		...
		{ 
		$query = $this->db->query("select count(*) as jml from subsubmenu");
        foreach ($query->result() as $row) {
            $jumlah = $row->jml;
        }
        $config['base_url'] = base_url().'/subsubmenu/index/';
        ...
        $data['artikel'] = $this->M_dataadmin->indexsubsubmenu('', $config['per_page'], $offset);
		...
		$data['judulapp']="Data Sub Submenu Front-End";
...
function add()
    {
		...
		//$data['vdata']='v_identitas';
		$data['judulapp']="Tambah Sub Submenu Front-End";
		$data['vnavigasi']='navigasi'; 
		$this->load->view('dashboard',$data);	 
    }
function edit()
    {
		...
		if ($leveluser=="admin" or $cek==1)
		{
			$query = $this->M_dataadmin->editsubsubmenu($this->uri->segment(3,0)); 
			$row = $query->row();
		...
		$data['judulapp']="Edit Sub Submenu Front-End";
		$data['vnavigasi']='navigasi'; 
		$this->load->view('dashboard',$data);	 
    }
...
function a_simpan()
    {
		...
		{	
			...
			$this->M_dataadmin->query_manual("INSERT INTO subsubmenu(id_menu, 
                                    id_submenu,
									nama_subsubmenu,
									aktif,
									css,
                            ...
                            VALUES('".$this->input->post('id_parent')."', 
								   '".$this->input->post('id_submenu')."',
                                   '".$this->input->post('nama_subsubmenu')."',
	...
	function a_edit()
    {
		...
		{  
			...
			$this->M_dataadmin->query_manual("UPDATE subsubmenu SET  id_menu = '".$this->input->post('id_parent')."',
									keterangan = '".$this->input->post('keterangan')."',
									css = '".$this->input->post('css')."',
									id_submenu = '".$this->input->post('id_submenu')."',
                                   nama_subsubmenu = '".$this->input->post('nama_subsubmenu')."',
                                   link_subsubmenu  = '".$this->input->post('link')."',
								   aktif = '".$this->input->post('aktif')."'
                             WHERE id_subsubmenu   = '".$kode."'");  
	...
	function delete()
    {
		...
		{  
			...
			$row = $query->row();
			if ($this->session->userdata('id_user') == $row->username or $leveluser=="admin") { 
				$this->M_dataadmin->query_manual("DELETE FROM subsubmenu WHERE id_subsubmenu=".$kode."");  
				//REDIRECT AWAL
				$this->redirect();
			}
			else {
	...
	function deleteall()
    {
		...
		{   
				...
				if ($leveluser=="admin") {
					for($i=0;$i<$jumlah;$i++){ 
						$this->M_dataadmin->query_manual("DELETE FROM subsubmenu WHERE id_subsubmenu='".$cek[$i]."'");    
					}  
				}
				else {
					for($i=0;$i<$jumlah;$i++){
						$this->M_dataadmin->query_manual("DELETE FROM subsubmenu WHERE username='".$this->session->userdata('id_user')."' and id_subsubmenu='".$cek[$i]."'");  
					} 
				} 
				}
				//REDIRECT AWAL
	...
	function urutall()
    {
		...
		{   
				...
				if ($leveluser=="admin") {
					for($i=0;$i<$jumlah;$i++){ 	
						$this->M_dataadmin->query_manual("UPDATE subsubmenu SET urutan = '".$urutan[$i]."' WHERE id_subsubmenu = '".$cek[$i]."'");   
						//"UPDATE subsubmenu SET urutan = '".$urutan[$i]."' WHERE id_subsubmenu = '".$cek[$i]."'"						
					}  
				}
				else {
					$this->redirect();
	...
	function aktif()
    {
		...
		$kode=$this->uri->segment(3,0);
			if ($leveluser=="admin") { 
				$this->M_dataadmin->query_manual("UPDATE subsubmenu SET aktif='Y' WHERE id_subsubmenu=".$kode."");  
				$this->redirect();
			}
			else {
				$query = $this->db->query("SELECT * FROM subsubmenu WHERE username='".$this->session->userdata('id_user')."' and id_subsubmenu='".$kode."'");
	...
	function nonaktif()
    {
		...
		$kode=$this->uri->segment(3,0);
			if ($leveluser=="admin") { 
				$this->M_dataadmin->query_manual("UPDATE subsubmenu SET aktif='N' WHERE id_subsubmenu=".$kode."");  
				$this->redirect();
			}
			else {
				$query = $this->db->query("SELECT * FROM subsubmenu WHERE username='".$this->session->userdata('id_user')."' and id_subsubmenu='".$kode."'");
	...
	function hapuslink()
    {
		...
		{  
			...
			$row = $query->row();
			if ($this->session->userdata('id_user') == $row->username or $leveluser=="admin") {  
				$this->M_dataadmin->query_manual("UPDATE subsubmenu SET link_subsubmenu='' WHERE id_subsubmenu=".$kode."");   
			}
			else {
				$this->denied();
			}  
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."subsubmenu/edit/".$kode."'>";
			}
		else {
			$this->denied();
		}  
	...
	public function linksubsubmenu () {  
	...
	public function pilihsubmenu () {  
		 
		$posisi = $this->input->post('id_parent'); 
		
		?> 
		<option value="0" selected>- Pilih Menu Utama -</option> 
			<?php
			$dataa = $this->M_dataadmin->pilihsubmenu($posisi); 
			foreach($dataa->result_array() as $raw) { 
			?> 
				<option  value="<?php echo $raw['id_submenu'];?>"><?php echo $raw['nama_submenu'];?></option>
			<?php } ?>	 
		<?php 
	}
	...
	public function redirect () {
		echo "<meta http-equiv='refresh' content='0; url=".base_url()."subsubmenu'>";
	}
