...
function a_simpan()
    {
		...
		{	
			...
			$d=substr($this->input->post('penetapan'), 6,9);
			$e=substr($this->input->post('penetapan'), 3,2);
			$f=substr($this->input->post('penetapan'), 0,2);
			$tanggalpenetapan=$d.'-'.$e.'-'.$f;
			...
			$ukuran_max_file=50000000; // Dalam bytes = 4 MB 
				...
				if (!empty($lokasi_file)){ 
					...
					$this->M_dataadmin->query_manual("insert into produkhukum (nomor,  
													metode_link,
													keterangan, 
													nama_file,
													link_file,
	...
	function a_edit()
    {
		...
		{  
			...
			if ($kondisi=='1'){
				...
				$row = $query->row();
				if ($row->nama_file!='') {
			$this->M_dataadmin->query_manual("UPDATE produkhukum SET nomor = '".$this->input->post('nomor')."',
