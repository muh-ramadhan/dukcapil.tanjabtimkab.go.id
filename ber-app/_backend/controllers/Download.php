...
function index()
    {
		...
		{ 
		$query = $this->db->query("select count(*) as jml from download");
        ...
        $data['artikel'] = $this->M_dataadmin->indexdownload('', $config['per_page'], $offset);
		...
		$data['judulapp']="Data File Download";
...
function add()
    {
		...
		//$data['vdata']='v_identitas';
		$data['judulapp']="Tambah File Download";
		$data['vnavigasi']='navigasi'; 
		$this->load->view('dashboard',$data);	 
    }
function edit()
    {
		...
		if ($leveluser=="admin" or $cek==1)
		{
			$query = $this->M_dataadmin->editdownload($this->uri->segment(3,0)); 
			$row = $query->row();
		...
		$data['judulapp']="Edit File Download";
		$data['vnavigasi']='navigasi'; 
		$this->load->view('dashboard',$data);	 
    }
...
function a_simpan()
    {
		...
		{	
			...
			$ukuran_max_file=500000000; // Dalam bytes = 4 MB 
				...
				if (!empty($lokasi_file)){ 
					...
					if ($file_extension=='php' || $file_extension=='exe' || $file_extension=='dll' || $file_extension=='ico'){
						...
						window.location=('".base_url()."download/add')</script>";
					}
					...
					$this->M_dataadmin->query_manual("insert into download (judul,  
													metode_link,
													keterangan, 
													nama_file,
													link_file,
		...
		{  
			...
			$ukuran_max_file=50000000; // Dalam bytes = 4 MB 
				...
				if (!empty($lokasi_file)){ 
					...
					if ($file_extension=='php' || $file_extension=='exe' || $file_extension=='dll' || $file_extension=='ico'){
						echo "<script>window.alert('Upload Gagal, Pastikan File yang di Upload tidak bertipe *.PHP / .*EXE');
						window.location=('".base_url()."download/edit/".$this->uri->segment(3,0)."')</script>";
					}
					elseif($size > $ukuran_max_file){          
						echo "<script>window.alert('Upload gagal! Ukuran File Anda $size bytes lebih dari $ukuran_max_file bytes, Terlalu Besar! Silahkan dikurangi');
						window.location=('".base_url()."download/edit/".$this->uri->segment(3,0)."')</script>";
					...
					$query = $this->db->query('SELECT nama_file, tanggal_modif FROM download WHERE id_download="'.$kode.'";');
					$row = $query->row();
					if ($row->nama_file!=''){
					...
					$this->M_dataadmin->query_manual("UPDATE download SET judul = '".$this->input->post('judul')."',
	...
	function delete()
    {
		...
		{  
			...
			if ($row->nama_file!=''){ 
				$pathdelete=str_replace("-","/",$row->tanggal_modif);
				unlink("../file/".$pathdelete."/".$row->nama_file); 
				$this->M_dataadmin->query_manual("DELETE FROM download WHERE id_download='".$kode."'"); 
			}
			else{
				$this->M_dataadmin->query_manual("DELETE FROM download WHERE id_download=".$kode.""); 
			}
	...
	function deleteall()
    {
		...
		{   
				...
				if ($leveluser=="admin") {
					for($i=0;$i<$jumlah;$i++){ 	
						$query = $this->db->query("SELECT nama_file,tanggal_modif FROM download WHERE id_download='".$cek[$i]."'");
						$row = $query->row();  
						if ($row->nama_file!=''){ 
							$pathdelete=str_replace("-","/",$row->tanggal_modif);
							unlink("../file/".$pathdelete."/".$row->nama_file); 
						}
						$this->M_dataadmin->query_manual("DELETE FROM download WHERE id_download='".$cek[$i]."'");    
					}  
				}
				else {
					for($i=0;$i<$jumlah;$i++){
					//echo $jumlah." - $cek[$i] - ".$cek[$i]; 
						$query = $this->db->query("SELECT nama_file,tanggal_modif FROM download WHERE username='".$this->session->userdata('id_user')."' and id_download='".$cek[$i]."'");
						$row = $query->row();  
						...
						if ($row->nama_file!=''){ 
							...
							$this->M_dataadmin->query_manual("DELETE FROM download WHERE username='".$this->session->userdata('id_user')."' and id_download='".$cek[$i]."'"); 
						}
					} 
				} 
				}
	...
	function hapuslink()
    {
		...
		$kode=$this->uri->segment(3,0);
			if ($leveluser=="admin") { 
				$this->M_dataadmin->query_manual("UPDATE download SET link_file='' WHERE id_download=".$kode."");  
				$this->redirect();
			}
			else {
				$query = $this->db->query("SELECT * FROM download WHERE username='".$this->session->userdata('id_user')."' and id_download='".$kode."'");
				$jumlah = $query->num_rows();
				if ($jumlah>0) {
					$this->M_dataadmin->query_manual("UPDATE download SET link_file='' WHERE id_download=".$kode."");
					//$this->redirect();
	...
	function hapusfile()
    {
		...
		$kode=$this->uri->segment(3,0);
			if ($leveluser=="admin") {
				$query = $this->db->query("SELECT nama_file,tanggal_modif FROM download WHERE username='".$this->session->userdata('id_user')."' and id_download='".$kode."'");
				$row = $query->row(); 
				$pathdelete=str_replace("-","/",$row->tanggal_modif);
				unlink("../file/".$pathdelete."/".$row->nama_file); 
				$this->M_dataadmin->query_manual("UPDATE download SET nama_file='' WHERE id_download=".$kode."");  
				echo "<meta http-equiv='refresh' content='0; url=".base_url()."download/edit/".$kode."'>";
			}
			else {
				$query = $this->db->query("SELECT nama_file,tanggal_modif FROM download WHERE username='".$this->session->userdata('id_user')."' and id_download='".$kode."'");
	...
	function aktif()
    {
		...
		$kode=$this->uri->segment(3,0);
			if ($leveluser=="admin") { 
				$this->M_dataadmin->query_manual("UPDATE download SET aktif='Y' WHERE id_download=".$kode."");  
				$this->redirect();
			}
			else {
				$query = $this->db->query("SELECT * FROM download WHERE username='".$this->session->userdata('id_user')."' and id_download='".$kode."'");
	...
	function nonaktif()
    {
		...
		$kode=$this->uri->segment(3,0);
			if ($leveluser=="admin") { 
				$this->M_dataadmin->query_manual("UPDATE download SET aktif='N' WHERE id_download=".$kode."");  
				$this->redirect();
			}
			else {
				$query = $this->db->query("SELECT * FROM download WHERE username='".$this->session->userdata('id_user')."' and id_download='".$kode."'");
	...
	public function redirect () {
		echo "<meta http-equiv='refresh' content='0; url=".base_url()."download'>";
	}
