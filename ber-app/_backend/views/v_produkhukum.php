...
<!--		 <form action="<?php echo base_url(); ?>produkhukum/a_deleteall" method="POST">	-->	
	...
	?>
	   <tr class="odd">
			...
			<td><a class="bold" href="<?php echo base_url(); ?>produkhukum/edit/<?php echo $row['id_produkhukum']."/".$judul."/";?>"> <?php echo $row['nomor']; ?></a></td> 
			...
			<?php } else {?>
				<a  target="_blank" href="<?php echo $row['link_file'];?>" >Download </a>
			<?php } ?> </td> 
...
<option  value="<?php echo $r['id_katprodukhukum'];?>"><?php echo $r['nama_katprodukhukum'];?></option>
 ...
 <div class="form-group">
								<label>Tentang </label> 
								<textarea class="form-control" rows="5"  name="tentang" ></textarea> 
							</div> 	
		
<div class="form-group">
						<label>Tanggal Ditetapkan</label> 
                <div class="input-group date" id="datetimepicker1" style="width:180px;" >
                    <input type="text" name="penetapan" class="form-control" value="" />
                    <span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span>
                    </span>
                </div>
            </div>
...
$('#linkmethod').load('<?php echo base_url()?>produkhukum/custom');
 ...
 <div class="form-group">
								<label>Bidang </label> 
								<textarea class="form-control" rows="3"  name="bidang" ></textarea> 
							</div>
                        ...
                        <div class="panel-body">
						 <div class="form-group">
							...
							<div class="form-group">
								...
								<textarea class="form-control" rows="6"  id="loko" name="keterangan" ></textarea> 
							</div> 
							
							<div class="form-group">
								<label>Abstrak </label> 
								<textarea class="form-control" rows="6" id="loko2"  name="abstrak" ></textarea> 
							</div> 
...
<script src='http://cdnjs.cloudflare.com/ajax/libs/bootstrap-datetimepicker/3.1.3/js/bootstrap-datetimepicker.min.js'></script>
 ...
 <div class="row">
	...
	<form method="POST" id="produkhukum" name="produkhukum" action="<?php echo base_url(); ?>produkhukum/a_edit"  enctype="multipart/form-data" onsubmit="return validasi(this)">					
		...
		if ($raw['tanggal_penetapan']!=null) {
		$d=substr($raw['tanggal_penetapan'], 0,4);
		$e=substr($raw['tanggal_penetapan'], 5,2);
		$f=substr($raw['tanggal_penetapan'], 8,9);
		$tanggalpenetapan=$f.'-'.$e.'-'.$d;
		}
		else {
		$tanggalpenetapan="";
		}
		...
		<?php } ?> 
			...
			<div class="form-group">
				<label>Nomor</label> 
				<input  class="form-control" type="text" name="nomor" value="<?php echo $raw['nomor']; ?>">
			</div>
...
<div class="form-group">
			...
			<div class="form-group">
	<label>Tentang </label> 
	<textarea class="form-control" rows="5"  name="tentang" ><?php echo $raw['tentang']; ?></textarea>
</div>
	<div class="form-group">
						<label>Tanggal Penetapan</label> 
                <div class="input-group date" id="datetimepicker1" style="width:180px;" >
                    <input type="text" name="penetapan" class="form-control" value="<?php echo $tanggalpenetapan; ?>" >
                    <span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span>
                    </span>
                </div>
            </div>		
...
<div class="form-group">
	<label>Bidang </label> 
	<textarea class="form-control" rows="3"  name="bidang" ><?php echo $raw['bidang']; ?></textarea> 
</div>
			 ...
			 <div class="col-lg-7">
				...
				<div class="form-group">
					<label>Status</label> 
					<textarea class="form-control"  rows="3"  name="status"> <?php echo $raw['status']; ?></textarea> 
				</div>
				
				<div class="form-group">
					<label>Keterangan </label> 
					<textarea class="form-control" rows="6"  id="loko"  name="keterangan" > <?php echo $raw['keterangan']; ?></textarea> 
				</div> 
							
				<div class="form-group">
					<label>Abstrak </label> 
					<textarea class="form-control" rows="6" id="loko2" name="abstrak" ><?php echo $raw['abstrak']; ?></textarea> 
				</div>  
							<div class="clearfix"></div>
						<div class="aksi" style="border-top:1px solid #ccc; margin-top:20px; ">
						<div class="clearfix"></div>
