...
$('.case').attr("checked", "checked");
	$('#deleturut').attr('action', '<?php echo base_url(); ?>submenu/urutall'); 
});

$('#buttondelet').click(function(){
	$('#deleturut').attr('action', '<?php echo base_url(); ?>submenu/deleteall'); 
});   
 ...
 <!-- Social Buttons CSS -->
    <link href="<?php echo base_url()?>style/bower_components/bootstrap-social/bootstrap-social.css" rel="stylesheet"> 
		 <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Data Submenu Front-End</h1>
                </div> 
            </div>
		<div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
							<i class="fa fa-th-list fa-fw"></i> Submenu Front-End Terbaru
                        </div> 
                        <div class="panel-body">
<!--		 <form action="<?php echo base_url(); ?>submenu/a_deleteall" method="POST">	-->	
	<?php //echo form_open_multipart('submenu/deleteall'); ?> 		 
	...
	<br> 
					 <center> 
						...
						<a href="<?php echo base_url(); ?>submenu/add" class="btn btn-app btn-primary btn-xs radius-4">
							<i class="ace-icon fa fa-plus-circle bigger-160"></i> Add
						</a>
						
						<a href="<?php echo base_url(); ?>submenu" class="btn btn-app btn-warning  btn-xs  radius-4">
	...
	foreach($artikel as $key => $row){ 
		$judul=seo_link($row['nama_submenu']); 	
	...
	?>
	   <tr class="odd">
			<td><?php echo $no; ?></td>
            <td><center><input type="checkbox" name="cek[]" class="case" value="<?php echo $row['id_submenu']; ?>" id="id<?php echo $no; ?>" rel="ck" title="Pilih"></center></td>
            <td><a class="bold" href="<?php echo base_url(); ?>submenu/edit/<?php echo $row['id_submenu']."/".$judul."/";?>"> <?php echo $row['nama_submenu']; ?> </a></td> 
			...
			<td><?php echo $row['link_submenu']; ?></td>
			<td>
			<?php if ($row['jangkrik']=='Y') {?>
				<a href="<?php echo base_url(); ?>submenu/nonaktif/<?php echo $row['id_submenu']."/".$judul."/";?>" class="btn btn-info btn-circle btn-lg"><i class="fa fa-check"></i> </a>
			<?php } else {?>
				<a href="<?php echo base_url(); ?>submenu/aktif/<?php echo $row['id_submenu']."/".$judul."/";?>" 	class="btn btn-warning btn-circle btn-lg"><i class="fa fa-times"></i> </a>
			<?php } ?> 
			</td>
            <td class="center"><?php echo $row['nama_lengkap']; ?></td>
            <td class="center">
				<a href="<?php echo base_url(); ?>submenu/edit/<?php echo $row['id_submenu']."/".$judul."/";?>" class="btn btn-block btn-social btn-dropbox">
					<i class="fa fa-pencil"></i> Edit
                </a> 
				
				<a href="<?php echo base_url(); ?>submenu/delete/<?php echo $row['id_submenu']."/".$judul."/";?>" class="btn btn-block btn-social btn-pinterest" onclick="return confirm('Apakah Anda benar-benar mau menghapusnya?')">
...
var prop = $("#posisi").val();
$.ajax({
url: "<?php echo base_url()?>submenu/pilihmenu",
type: 'POST',
data:  $("#menu").serialize(), 
//url: "pilihmenu.php",
...
var prop = $("#metode").val();
$.ajax({ 
url: "<?php echo base_url()?>submenu/linksubmenu",
type: 'POST',
data:  $("#menu").serialize(), 
cache: false,
success: function(msg){ 
...
<script src='http://cdnjs.cloudflare.com/ajax/libs/bootstrap-datetimepicker/3.1.3/js/bootstrap-datetimepicker.min.js'></script>
 ...
 <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Tambah Submenu Front-End</h1>
                </div> 
            </div>
            ...
            <div class="row">
                <div class="col-lg-10">
                    <div class="panel panel-default">
                        <div class="panel-heading">
							<i class="fa fa-wrench fa-fw"></i> Tambah Submenu Front-End 
                        </div> 
                        <div class="panel-body"> 
						 

<form method="POST" id="menu" name="menu" action="<?php echo base_url(); ?>submenu/a_simpan"  enctype="multipart/form-data" onsubmit="return validasi(this)">		 
			...
			<div class="form-group">
                                            <label>Publish Submenu Front-End</label>
                                            <div class="radio">
                                                <label>
							...
							<div class="form-group">
								...
								<input  class="form-control" type="text" name="nama_submenu" value="">
		...
		<div class="clearfix"></div>
			...
			<div class="form-group">
				<label>Pilih Posisi Submenu</label> 
				<select class="form-control"  name="posisi" id="posisi">
					<option value="0" selected>- Pilih Posisi Submenu -</option>
					<?php
			...
			<div class="form-group">
				<label>Pilih Menu Utama (Front-End)</label> 
				 
				<select class="form-control"  name="id_parent" id="id_parent">
					<option value="0" selected>- Pilih Menu Utama -</option>
				<!--	
					<?php
					$dataa = $this->M_dataadmin->pilihmenu($raw['id_menu']); 
					foreach($dataa->result_array() as $raw) {
					?> 
					<option  value="<?php echo $raw['id_menu'];?>"><?php echo $raw['nama_posisi'];?></option>
					<?php } ?>
				-->	
                </select>
			</div>
			...
			<div class="form-group">
				<label>Pilih Link Submenu</label> 
				<select class="form-control" name="metode" id="metode">
				<option value="0" selected>- Pilih Link Submenu -</option>
				<?php
...
<script src='http://cdnjs.cloudflare.com/ajax/libs/bootstrap-datetimepicker/3.1.3/js/bootstrap-datetimepicker.min.js'></script>
 ...
 <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Edit Submenu Front-End</h1>
                </div> 
            </div>
            ...
            <div class="row">
                <div class="col-lg-10">
                    <div class="panel panel-default">
                        <div class="panel-heading">
							<i class="fa fa-wrench fa-fw"></i>Edit Submenu Front-End 
                        </div> 
                        <div class="panel-body"> 
					 
	<form method="POST" id="menu" name="menu" action="<?php echo base_url(); ?>submenu/a_edit"  enctype="multipart/form-data" onsubmit="return validasi(this)">					
		<?php
		$edit = $this->M_dataadmin->editsubmenu($this->uri->segment(3,0)); 
		...
		?>
						<input type="hidden" name="id" value="<?php echo $raw['id_submenu']; ?>">  
						<div class="aksi" style="border-bottom:1px solid #ccc; margin-bottom:20px; ">
						<center>
						<button type="submit" class="btn btn-app btn-primary btn-xs radius-4">
							<i class="ace-icon fa fa-floppy-o bigger-160"></i> Save
		...
		<?php } ?> 
			...
			<div class="form-group">
				<label>Nama Submenu</label> 
				<input  class="form-control" type="text" name="nama_submenu" value="<?php echo $raw['nama_submenu']; ?>">
			</div>
			<div class="clearfix"></div>
		<div style="border:1px solid #ec8585;background:#f2f2f2;border-radius:5px;padding:20px;margin-bottom:20px;width:300px;">
			<div class="form-group">
	...
	<div class="form-group">
		<label>Pilih Posisi Submenu</label> 
			<select class="form-control"  name="posisi" id="posisi"> 
						...
						$dataa = $this->M_dataadmin->pilihposisimenu(); 
						if ($posisi==0){
						echo "<option value=0 selected>- Pilih Posisi Submenu -</option>";
						}   
		...
		<label>Link Submenu</label>
			<input value="<?php echo $raw['link_submenu']; ?>" class="form-control"  type="text" name="link" >
			<div class="clearfix"></div>
			...
			<div class="clearfix"></div>
			<a href="<?php echo base_url(); ?>submenu/hapuslink/<?php echo $raw['id_submenu'];?>" class="btn btn-block btn-social btn-pinterest" style="display:inline;" onclick="return confirm('Apakah Anda benar-benar mau menghapusnya?')"> <i class="fa fa-trash-o"></i> Hapus Link </a>
