...
define("TIME_ZONE","+07:00");
	...
	public function getMenu2($parent,$hasil){
        ...
        if(($w->num_rows())>0)
        {
            $hasil.= " <ul id='nava' class='nav nav-horizontal clearfix'> <div class='imglogo'><a href=".base_url()."><img src='".base_url()."style/images/logo-small.png'></a></div> ";
        }
			...
			if ($wa->num_rows()>0) {
			$hasil.= "<li><a href='".$link1."'> ".$h->nama_menu."<img src='".base_url()."/style/images/green-arrow.png' style='margin-left:5px;*margin-top:-100px;'></a><ul>";
			}
			else {
			$hasil.= "<li><a href='".$link1."'> ".$h->nama_menu."</a><ul>";
			}
			...
			if ($lin2!="htt") {
				...
				$link2=$ha->link_submenu;
							...
							$hasil.= "<li><a href='".$link2."'> ".$ha->nama_submenu."</a><ul>"; 
								$waa= $this->db->query("SELECT * from subsubmenu where id_submenu='".$ha->id_submenu."'  and aktif='Y' order by urutan");
								foreach($waa->result() as $haa)
									{
			$lin3=substr($haa->link_subsubmenu, 0, 3);  
			if ($lin3!="htt") {
				$link3=base_url().$haa->link_submenu;
			}
			else {
				$link3=$haa->link_submenu;
			}	
										$hasil .= "<li><a href='".$link3."'> ".$haa->nama_subsubmenu."</a></li>"; 
									}
	...
	public function getMenu1($parent,$hasil){
        ...
        if(($w->num_rows())>0)
        {
			...
			$hasil.= "<li><a href=".base_url().$h->link."> ".$h->nama_menu."</a><ul>";
					...
					foreach($wa->result() as $ha)
						{
							$hasil.= "<li><a href=".base_url().$ha->link_submenu."> ".$ha->nama_submenu."</a><ul>"; 
								$waa= $this->db->query("SELECT * from subsubmenu where id_submenu='".$ha->id_submenu."'  and aktif='Y' order by urutan");
								foreach($waa->result() as $haa)
									{
										$hasil .= "<li><a href=".base_url().$haa->link_subsubmenu."> ".$haa->nama_subsubmenu."</a></li>"; 
									}
				...
				$hasil.= " </ul></li>";
			$hasil = $this->getMenu1($h->id_menu,$hasil);
...
*/
		...
		public function ambilsubsubmenu ($idmenu,$idsubmenu)
		{
			//id_position='".$posisi."' and 
			$sql="select * from subsubmenu where  aktif='Y' and  id_submenu='".$idsubmenu."' order by urutan asc";
			$hslquery=$this->db->query($sql);
			return $hslquery;
		}	
		...
		public function ambilbukuhukum ($limit)
		{
			$sql="select * from buku where aktif='Y' order by id_buku desc limit $limit";
			$hslquery=$this->db->query($sql);
			return $hslquery;
		}
		...
		*/
		...
		public function pengumuman ($limit)
		{
			...
			return $hslquery;
				...
				public function arthuk ($limit)
		{
			$sql="select * from artikel where aktif='Y' order by id_artikel desc limit $limit";
			$hslquery=$this->db->query($sql);
			return $hslquery;
		}
        ...
        {
            $hasil .= "<br>
			<button class='ltu-button' type='submit'>Submit</button> </form>
			<a href='".base_url()."polling/' class='ltu-button' >Lihat Hasil</a>
			</div>";
			//type="submit"
			//$hasil.= "<li><a href=".base_url().$h->link."> ".$h->nama_menu."</a><ul>";
        }
	...
	/*--------------------- AWAL PRODUK HUKUM -------------------*/
		...
		{
			$sql="select * from katprodukhukum where aktif='Y' order by urutan asc";
			$hslquery=$this->db->query($sql);
			return $hslquery;
		}
		
		//produkhukum by kategori
		...
		//produkhukum by kategori dan tahun
		public function getprodukhukumkattahun($catid, $perPage, $uri, $tahun) {
        $data = array();
        $this->db->select('*,produkhukum.keterangan as jangkrik');
        $this->db->from('produkhukum');
        $this->db->where("produkhukum.id_katprodukhukum", $catid);
		...
		//produkhukum by tahun dan kategori
		public function getprodukhukumtahunkat($catid, $perPage, $uri, $tahun) {
        $data = array();
        $this->db->select('*,produkhukum.keterangan as jangkrik');
        $this->db->from('produkhukum');
        $this->db->where("produkhukum.id_katprodukhukum", $catid);
		...
		//produkhukum by tahun
		public function getprodukhukumtahun($tahun, $perPage, $uri ) {
        $data = array();
        $this->db->select('*,produkhukum.keterangan as jangkrik');
        $this->db->from('produkhukum');
        //$this->db->where("produkhukum.id_katprodukhukum", $catid);
		...
		{
			$query = $this->db->query('SELECT * FROM produkhukum left join katprodukhukum on katprodukhukum.id_katprodukhukum=produkhukum.id_katprodukhukum WHERE id_produkhukum='.$id.';');
			$row = $query->row(); //takes only one result row
			$wartosc = $row->nama_katprodukhukum." Nomor ".$row->nomor." Tahun ".$row->tahun;
			return $wartosc; 
		}
	...
	public function jumlahperkategori($id)
	{ 
		$q =$this->db->query("select * from produkhukum where id_katprodukhukum='".$id."' and aktif='Y' ");
		return $q->num_rows(); 
	}
	
	public function jumlahpertahun($id)
	{ 
		$q =$this->db->query("select * from produkhukum where tahun='".$id."' and aktif='Y' ");
		return $q->num_rows(); 
	}

	public function jumlahpertahundankategori($id,$tahun)
	{ 
		$q =$this->db->query("select * from produkhukum where id_katprodukhukum='".$id."' and tahun='".$tahun."' and aktif='Y' ");
		return $q->num_rows(); 
	}	
	 
	public function grouptahunprodukhukum($id)
	{
		$sql="select * from produkhukum where id_katprodukhukum='".$id."' and aktif='Y' group by tahun asc";
		$hslquery=$this->db->query($sql);
		return $hslquery;
	}
	
	public function groupprodukhukumtahun($id)
	{
		$sql="select * from produkhukum left join katprodukhukum on katprodukhukum.id_katprodukhukum=produkhukum.id_katprodukhukum where tahun='".$id."' and produkhukum.aktif='Y' group by produkhukum.id_katprodukhukum asc";
		$hslquery=$this->db->query($sql);
		return $hslquery;
	}
	
	public function grouptahunprodukhukum2()
	{
		$sql="select * from produkhukum where  aktif='Y' group by tahun asc";
		$hslquery=$this->db->query($sql);
		return $hslquery;
	}
	function cariperaturan($kata) {
		$kata = htmlentities(htmlspecialchars($kata), ENT_QUOTES);
		  $pisah_kata = explode(" ",$kata);
		  $jml_katakan = (integer)count($pisah_kata);
		  $jml_kata = $jml_katakan-1;
		  ...
		  $hsl = array();
			$sql="SELECT *,produkhukum.keterangan as keterangan FROM produkhukum left join katprodukhukum on katprodukhukum.id_katprodukhukum=produkhukum.id_katprodukhukum  WHERE " ;
			//for ($i=0; $i<=$jml_kata; $i++){
			//$sql .= "tentang LIKE '%$pisah_kata[$i]%'";
			//if ($i < $jml_kata ){
			//$sql .= " OR "; } }
			
			
			$sql .= "tentang LIKE '%$kata%'";
			$sql .= " and produkhukum.aktif='Y' ORDER BY id_produkhukum desc";
			$hslquery=$this->db->query("$sql");
			if($hslquery->num_rows() > 0){
				$hsl=$hslquery->result();
			}
	...
	public function getData($tentang)
	{
		$this->db->select('tentang');
		$this->db->like('tentang', $tentang);
		$this->db->group_by('tentang');
		$query = $this->db->get('produkhukum');
		return $query->result();
	} 	 
	
	
	
	/*--------------------- AWAL MODULE propemperda -------------------*/
		 
		
		public function getpropemperda($catid, $perPage, $uri) {
        $data = array();
        $this->db->select('*');
        $this->db->from('propemperda'); 
        $this->db->order_by('id_propemperda', 'desc'); 
        $this->db->where('aktif', 'Y');
        $getData = $this->db->get('', $perPage, $uri);
        if ($getData->num_rows() > 0)
            return $getData->result_array();
		...
		/*
		public function judulpropemperda ( $id='' )
		{
			$query = $this->db->query('SELECT judul FROM propemperda WHERE id_propemperda='.$id.';');
			$row = $query->row(); //takes only one result row
			$wartosc = $row->judul;
			return $wartosc; 
		}
		
		public function idpropemperda ($id='' )
		{
			$hsl = array();
			$sql="select * from propemperda where aktif='Y' and id_propemperda='".$id."'";
			$hslquery=$this->db->query("$sql");
			if($hslquery->num_rows() > 0){
				$hsl=$hslquery->result();
			}
	...
	/*--------------------- AWAL MODULE raperda -------------------*/
		...
		public function getraperda($catid, $perPage, $uri) {
        $data = array();
        $this->db->select('*');
        $this->db->from('raperda'); 
        $this->db->order_by('id_raperda', 'desc'); 
        $this->db->where('aktif', 'Y');
        $getData = $this->db->get('', $perPage, $uri);
        if ($getData->num_rows() > 0)
            return $getData->result_array();
		...
		public function judulraperda ( $id='' )
		{
			$query = $this->db->query('SELECT * FROM raperda WHERE id_raperda='.$id.';');
			$row = $query->row(); //takes only one result row
			$wartosc = 'Raperda No. '.$row->nomor.' Tahun '.$row->tahun;
			return $wartosc; 
		}
		
		public function idraperda ($id='' )
		{
			$hsl = array();
			$sql="select * from raperda where aktif='Y' and id_raperda='".$id."'";
			$hslquery=$this->db->query("$sql");
			if($hslquery->num_rows() > 0){
				$hsl=$hslquery->result();
			}
	...
	/*--------------------- AWAL MODULE naskahakademik -------------------*/
		 
		
		public function getnaskahakademik($catid, $perPage, $uri) {
        $data = array();
        $this->db->select('*');
        $this->db->from('naskahakademik'); 
        $this->db->order_by('id_naskahakademik', 'desc'); 
        $this->db->where('aktif', 'Y');
        $getData = $this->db->get('', $perPage, $uri);
        if ($getData->num_rows() > 0)
            return $getData->result_array();
		...
		public function judulnaskahakademik ( $id='' )
		{
			$query = $this->db->query('SELECT * FROM naskahakademik WHERE id_naskahakademik='.$id.';');
			$row = $query->row(); //takes only one result row
			$wartosc = $row->judul;
			return $wartosc; 
		}
		
		public function idnaskahakademik ($id='' )
		{
			$hsl = array();
			$sql="select * from naskahakademik where aktif='Y' and id_naskahakademik='".$id."'";
			$hslquery=$this->db->query("$sql");
			if($hslquery->num_rows() > 0){
				$hsl=$hslquery->result();
			}
	...
	/*--------------------- AWAL MODULE PUSAT -------------------*/
		...
		public function getpusat($catid, $perPage, $uri) {
        $data = array();
        $this->db->select('*,produkhukum.keterangan as jangkrik');
        $this->db->from('produkhukum');
        $this->db->where("produkhukum.id_katprodukhukum in (1,13,14)");
        $this->db->order_by('produkhukum.id_produkhukum', 'desc');
		$this->db->join('katprodukhukum', 'katprodukhukum.id_katprodukhukum = produkhukum.id_katprodukhukum', 'left');
        $this->db->where('produkhukum.aktif', 'Y');
        $getData = $this->db->get('', $perPage, $uri);
		...
		public function getjabar($catid, $perPage, $uri) {
        $data = array();
        $this->db->select('*,produkhukum.keterangan as jangkrik');
        $this->db->from('produkhukum');
        $this->db->where("produkhukum.id_katprodukhukum in (17,16)");
        $this->db->order_by('produkhukum.id_produkhukum', 'desc');
		$this->db->join('katprodukhukum', 'katprodukhukum.id_katprodukhukum = produkhukum.id_katprodukhukum', 'left');
        $this->db->where('produkhukum.aktif', 'Y');
        $getData = $this->db->get('', $perPage, $uri);
		...
		public function getsukabumikab($catid, $perPage, $uri) {
        $data = array();
        $this->db->select('*,produkhukum.keterangan as jangkrik');
        $this->db->from('produkhukum');
        $this->db->where("produkhukum.id_katprodukhukum in (2,4,10)");
        $this->db->order_by('produkhukum.id_produkhukum', 'desc');
		$this->db->join('katprodukhukum', 'katprodukhukum.id_katprodukhukum = produkhukum.id_katprodukhukum', 'left');
        $this->db->where('produkhukum.aktif', 'Y');
        $getData = $this->db->get('', $perPage, $uri);
...
public function getbuku($catid, $perPage, $uri) {
        $data = array();
        $this->db->select('*');
        $this->db->from('buku');
        $this->db->order_by('id_buku', 'desc');
		$this->db->join('jenisbuku', 'jenisbuku.id_jenisbuku=buku.id_jenisbuku', 'left');
        $getData = $this->db->get('', $perPage, $uri);
        if ($getData->num_rows() > 0)
            return $getData->result_array();
        else
...
public function jenisbuku ( $id='' )
		{
			$query = $this->db->query('SELECT nama_jenisbuku FROM jenisbuku WHERE id_jenisbuku='.$id.';');
			$row = $query->row(); //takes only one result row
			$wartosc = $row->nama_jenisbuku;
			return $wartosc; 
		}
		
				public function getbukukat($catid, $perPage, $uri) {
        $data = array();
        $this->db->select('*');
        $this->db->from('buku');
        $this->db->where("id_jenisbuku", $catid);
        $this->db->order_by('id_buku', 'desc');
		$this->db->join('users', 'users.username = buku.username', 'left');
        $this->db->where('aktif', 'Y');
        $getData = $this->db->get('', $perPage, $uri);
        if ($getData->num_rows() > 0)
            return $getData->result_array();
		...
		public function juduljenisbuku ( $id='' )
		{
			$query = $this->db->query('SELECT nama_jenisbuku FROM jenisbuku WHERE id_jenisbuku='.$id.';');
			$row = $query->row(); //takes only one result row
			$wartosc = $row->nama_jenisbuku;
		...
		{
			$hsl = array();
			$sql="select *,buku.deskripsi as jangkrik from buku left join jenisbuku on jenisbuku.id_jenisbuku=buku.id_jenisbuku where id_buku='".$id."'";
			$hslquery=$this->db->query("$sql");
			if($hslquery->num_rows() > 0){
				$hsl=$hslquery->result();
			}
		...
		{
			$sql="SELECT *,buku.tanggal_modif as fotbuk FROM buku  WHERE id_buku='".$id."'";
			$hslquery=$this->db->query($sql);
			return $hslquery;
		}	
public function sambutan ()
	{
		$sql="SELECT * FROM sambutan order by id_sambutan limit 1";
		$hslquery=$this->db->query($sql);
		return $hslquery;
	}		
